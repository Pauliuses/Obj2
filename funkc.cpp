...
void data::readData(vector<data> &list, string filename){
...
void Groups(vector<data>& list, vector<data>& Nlist)
{
    ...
    sort(list.begin(), list.end(), [](data& f, data& s) { return f.GetGalutinis() < s.GetGalutinis(); });

    auto index = find_if(list.begin(), list.end(), []( data& temp){
